...
impl MsEdgeTTS {
    ...
    fn process_binary_message(&mut self, data: &[u8], stream: &mut Vec<u8>) {
        let msg = String::from_utf8_lossy(data);
        if msg.contains("Path:audio") {
            let binary_delim = b"Path:audio\r\n";
            if let Some(start_idx) =
                data.windows(binary_delim.len()).position(|slice| slice == binary_delim)
            {
    ...
    fn build_ssml(&self) -> String {
        if self.raw_ssml {
            ...
            "#,
                self.language, self.speaker, self.rate, self.pitch, self.volume, self.text
            )
        }
    }
}
...
mod tests {
    use super::*;
    #[tokio::test]
    #[ignore] // 需要网络连接，可能被 API 限制（403 错误），仅在本地测试时运行
    async fn test_request() {
        // 注意：此测试可能因为 API 限制（403）而失败，这是正常的
        // 在实际使用中，Edge TTS API 可能需要特定的 User-Agent 或其他头部
        let mut tts = MsEdgeTTS::new(
            "".to_string(),
            "".to_string(),
            "".to_string(),
            "".to_string(),
        ...
        match result {
            ...
            Err(e) => {
                println!(
                    "Failed to receive response (may be due to API restrictions): {:?}",
                    e
                );
                // 不 panic，因为 API 可能被限制
            }
        }
    }
}
